...
class ObservedCube(object):
    ...
    def deredden(self):
        if self.EBV == 0.0:
            log.warn('E(B-V) = 0, not dereddening spectra.')
            return
        log.debug('Dereddening spectra, E(B-V) = %f.' % self.EBV)
    ...
    def bin(self, bin_size, cov_factor=None):
        log.debug('Binning cube (%d x %d), cov. factor=%.2f.' % (bin_size, bin_size, cov_factor))
    ...
    def toRestFrame(self):
        if self.inRestFrame:
            log.warn('Already in rest frame (z=%.2f).' % self.redshift)
            return
        log.debug('Putting spectra in rest frame (z=%.2f).' % self.redshift)
    ...
    def toAirWavelength(self):
        if not self.vaccuum_wl:
            log.warn('Already in air wavelengths.')
            return
    ...
    def resample(self, l_ini, l_fin, dl, vectorized=False):
        log.debug('Resampling spectra in [%.1f, %.1f], dl=%.2f \AA.' % (l_ini, l_fin, dl))
    ...
    def flagLowSN(self, llow_sn, lupp_sn, sn_min, convex_hull=False):
        log.debug('Masking pixels with S/N < %.1f (%.1f-%.1f AA)' % (sn_min, llow_sn, lupp_sn))
...
def preprocess_obs(obs, cfg):
    ...
    obs.header.extend(obs.wcs.to_header(), update=True)
