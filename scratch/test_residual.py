...
def plot_spectra(f_obs, f_syn, f_res, f_res_rect, f_err, at_flux, ll, yy, xx, galaxy_id, center):
    ...
    plt.subplot(313)
    plt.pcolormesh(ll, yy, (f_res_rect[:, :, x_slice] * 100 / f_syn[:, :, x_slice]).T, vmin=-err_lim, vmax=err_lim, cmap='RdBu')
    plt.ylabel(r'dec. [arcsec]')
    plt.ylim(yy.min(), yy.max())
    plt.xlabel(r'Wavelength [$\AA$]')
    plt.xlim(ll.min(), ll.max())
    plt.title('rectified residual [%]')
    ...
    r_rect = f_res_rect[:, center[1], center[2]] * 100.0 / f
    err = f_err[:, center[1], center[2]] * 100.0 / f
    plt.plot(ll, r, 'm-', label='residual')
    plt.plot(ll, r_rect, 'g-', label='residual (rectified)')
...
def smooth_spectra(fwhm, ll, flux):
...
f_res = f_obs - f_syn
f_res_rect = f_res - smooth_spectra(15.0, ll, f_res)
f_err = rms_box(100.0, ll, f_res_rect, threshold=0.5)
