...
class DobbyAdapter(object):
    ...
    def updateELines(self, elines, spec, j, i=None):
        ...
        if i is None:
            self.El_F[:, j] = elines['El_F']
            self.El_v0[:, j] = elines['El_v0']
            self.El_vd[:, j] = elines['El_vd']
            self.El_flag[:, j] = elines['El_flag']
            self.El_EW[:, j] = elines['El_EW']
            self.El_vdins[:, j] = elines['El_vdins']
            self.El_lcrms[:, j] = elines['El_lcrms']
            self.El_lc[:, j] = spec['total_lc']
        else:
    ...
    def getArgs(self, j, i=None):
        if i is None:
            f_obs = self.f_obs[:, j]
            f_syn = self.f_syn[:, j]
            f_err = self.f_err[:, j]
            if self.correct_good_frac and self._c.seg_good_frac is not None:
                gf = self._c.seg_good_frac[:, j]
                f_obs *= gf
                f_syn *= gf
                f_err *= gf
            f_res = f_obs - f_syn
            f_flagged = ((flags.no_starlight & self.f_flag[:, j]) > 0)
            f_res[f_flagged] = np.ma.masked
            return j, f_res, f_syn, f_err, self.v_0[j], self.v_d[j]
        else:
    ...
    def __iter__(self):
        if self._c.hasSegmentationMask:        
            for k in range(self._c.Nzone):
                yield self.getArgs(k)
        else:
...
def fit_spaxel_kwargs(ii, f_res, f_syn, f_err, stellar_v0, stellar_vd, kwargs):
    return fit_spaxel(ii, f_res, f_syn, f_err, stellar_v0, stellar_vd, **kwargs)
###############################################################################


###############################################################################
def fit_spaxel(ii, f_res, f_syn, f_err, stellar_v0, stellar_vd,
    ...
    if isinstance(ii, list):
        ...
        iz = ii
        name = suffix + '.' + name_template % (iz)

    outfile = path.join(tmpdir, '%s.hdf5' % name)

    Nmasked = np.ma.getmaskarray(f_res).sum()
    if (Nmasked / len(f_res)) > 0.5:
        log.debug(f'Skipping masked spaxel/zone {ii}')
        return ii, None, None

    if path.exists(outfile):
        try:
            elines, spec = read_summary_from_file(outfile)
            log.debug('fRead results for spaxel/zone {ii} from {outfile}.')
            return ii, elines, spec
        except:
            log.debug('Corrupt result file ({outfile}), removing and repeating the fit.')
            unlink(outfile)

    # Using stellar v0 and vd from the spaxel spectrum (even though from the integrated is usually safer)
    log.info(f'Fitting spaxel/zone {ii} with stellar_vd = {stellar_vd}')
...
if __name__ == '__main__':
    ...
    with MPIPoolExecutor(args.maxWorkers) as executor:
        while True:
            ...
            if da._c.hasSegmentationMask:
                for k, elines, spec in fit_result:
                    log.debug('Saving fit for zone [%d].' % (k))
                    da.updateELines(elines, spec, k)
            else:
                for j, i, elines, spec in fit_result:
                    log.debug('Saving fit for spaxel [%d, %d].' % (j, i))
                    da.updateELines(elines, spec, j, i)
